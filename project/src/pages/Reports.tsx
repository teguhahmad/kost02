...
const Reports: React.FC = () => {
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Period: ${format(parseISO(dateRange.start), 'dd MMM yyyy')} - ${format(parseISO(dateRange.end), 'dd MMM yyyy')}`, 15, 25);

    // Add summary
    doc.text('Summary:', 15, 35);
    doc.text(`Total Revenue: ${formatCurrency(totalRevenue)}`, 20, 45);
    doc.text(`Average Monthly Revenue: ${formatCurrency(averageRevenue)}`, 20, 55);
    doc.text(`Pending Payments: ${formatCurrency(totalPending)}`, 20, 65);
    doc.text(`Overdue Payments: ${formatCurrency(totalOverdue)}`, 20, 75);
    ...
    (doc as any).autoTable({
      startY: 85,
      head: [['Month', 'Revenue', 'Pending', 'Overdue', 'Occupancy Rate']],
      body: tableData,
      theme: 'grid',
  ...
  const handleExportCSV = () => {
    const headers = ['Month', 'Revenue', 'Pending', 'Overdue', 'Occupancy Rate'];
